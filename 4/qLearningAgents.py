...
class PacmanDeepQAgent(PacmanQAgent):
    ...
    def compute_q_targets(self, minibatch, network = None, target_network=None, doubleQ=False):
        ...
        nextQ = network.run(next_states)
        maxQ = np.choose(np.argmax(nextQ.data, axis = 1), target_network.run(next_states).data.T)

        explorationBonus = 1.0 / (2.0 * np.sqrt(self.counts[states_np[:,0], states_np[:,1]] / 100.0))
        Q_target[np.arange(len(minibatch)), actions] = explorationBonus + rewards + self.discount * (1 - dones) * maxQ 
        "*** YOUR CODE ENDS HERE ***"
